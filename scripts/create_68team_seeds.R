...
# and adds the 4 First Four play-in pairs. Template uses W16, W11, X11, Z16.
#
# Usage:
...
#   W16a, W16b, W11a, W11b, X11a, X11b, Z16a, Z16b
# =============================================================================
...
PLAYIN_SEEDS <- c("W16", "W11", "X11", "Z16")  # 2026: MW has W16+W11, West has X11, South has Z16
PLAYIN_SPLIT <- list(
  W16 = c("W16a", "W16b"),
  W11 = c("W11a", "W11b"),
  X11 = c("X11a", "X11b"),
...
#' @param playin_teams Named list: W16=c(id1,id2), W11=c(...), X11=c(...), Z16=c(...)
#'   Or integer vector of length 8: c(W16a,W16b, W11a,W11b, X11a,X11b, Z16a,Z16b)
...
create_68team_seeds <- function(seeds_64, season, playin_teams = NULL) {
  ...
  if (!is.null(playin_teams)) {
    if (is.numeric(playin_teams) && length(playin_teams) == 8) {
      playin_teams <- list(
        ...
        X11 = playin_teams[5:6], Z16 = playin_teams[7:8]
      )
    }
  ...
  if (nrow(playin_rows) == 0) {
    ...
    message("Added play-in placeholder rows. Edit the CSV to fill TeamIDs for W16a/b, W11a/b, X11a/b, Z16a/b.")
  }
  ...
  if (nrow(playin_rows) > 0) {
    ...
    if (nrow(bad) > 0) {
      warning("Teams in play-ins (W16/Z16) also appear as X16/Y16. ",
              "Fix seeds so direct 16-seeds are not also play-in teams.")
    }
  }
  result
}
...
main <- function(season = 2026L, playin_teams = NULL) {
...
# With play-in TeamIDs: Rscript scripts/create_68team_seeds.R 2026 W16a W16b W11a W11b X11a X11b Z16a Z16b
args <- commandArgs(trailingOnly = TRUE)
season <- if (length(args) >= 1) as.integer(args[1]) else 2026L
playin <- if (length(args) >= 9) as.integer(args[2:9]) else NULL  # W16a,W16b, W11a,W11b, X11a,X11b, Z16a,Z16b
