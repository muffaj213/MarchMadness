...
build_matchup_data <- function(tourney_results, seeds, win_pct, points_stats) {
  # Add seed numbers
  seeds <- seeds %>%
  ...
  games %>%
    select(
      Season, TeamA, TeamB, outcome,
      seed_diff, winpct_diff, rating_diff, pf_diff
    )
}
...
compute_matchup_features <- function(team_a, team_b, season, seeds, win_pct, points_stats) {
